...
- Added a read-only preview that reports affected servers, exact rollup bucket count, safe cleanup boundary, and bounded anomaly samples before any data can be removed.
...
- Excludes the recent mutable metric window from cleanup and requires the exact preview boundary for deletion.
- Requires explicit confirmation plus the preview's expected match count; stale previews fail closed and must be scanned again.
...
- Added storage-engine regression tests covering anomaly detection, targeted deletion, unrelated-metric preservation, and cleanup blast-radius refusal.
